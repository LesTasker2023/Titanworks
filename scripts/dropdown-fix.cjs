...
describe('DropdownMenu', () => {
  const BasicDropdownMenu = ({ children, ...props }: { children?: React.ReactNode; [key: string]: any }) => (
    <DropdownMenu>
  ...
  describe('Props', () => {
    it('handles children prop correctly', () => {
      render(<BasicDropdownMenu><DropdownMenuItem>Custom Item</DropdownMenuItem></BasicDropdownMenu>);
      expect(screen.getByTestId('dropdown-trigger')).toBeInTheDocument();
    });
  });
...
const filePath = 'src/components/ui/DropdownMenu/DropdownMenu.test.tsx';

try {
  fs.writeFileSync(filePath, newContent);
  console.log('✅ DropdownMenu test file completely rewritten with proper structure');
} catch (error) {
  console.error('❌ Error rewriting file:', error.message);
}
