...
async function fixSliderIssues() {
  ...
  content = content.replace(/formatValue: 'test-format'/g, 'formatValue: (value) => `${value}%`');
...
function shouldSkipFocusTests(componentName, content) {
  ...
  const nonFocusableComponents = ['Skeleton', 'Table', 'Tabs', 'Badge', 'Card', 'Avatar', 'Label'];
  ...
  if (content.includes('data-testid=') && (content.includes('<div') || content.includes('<span'))) {
    return true;
  }
